...
case "$1" in
    build)
        docker build --platform linux/amd64 -t darkfate .
        ;;
    run)
        # Ensure the container is not already running
        if [ "$(docker ps -q -f name=darkfate)" ]; then
            echo "Container 'darkfate' is already running. Stopping it first."
            docker stop darkfate
            docker rm darkfate
        fi
        ...
        CMD="$CMD --name darkfate darkfate"

        # Execute the command
        eval $CMD
        ;;
    start)
        docker start darkfate
        ;;
    bash)
        # Check if the container is running before executing bash
        if [ "$(docker ps -q -f name=darkfate)" ]; then
            docker exec -it darkfate bash
        else
            echo "Container 'darkfate' is not running. Please start it first."
            exit 1
        fi
        ;;
    *)
