    ...
    implements IDatabaseCacheAdapter
{
    ...
    constructor(connectionConfig: any) {
        ...
        this.pool.on("error", (err) => {
            darkfateLogger.error("Unexpected pool error", err);
        ...
        for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
            try {
                ...
                if (attempt < this.maxRetries) {
                    ...
                    darkfateLogger.error("Max retry attempts reached:", {
                        error:
                            error instanceof Error
                                ? error.message
                                : String(error),
    ...
    private async handlePoolError(error: Error) {
        darkfateLogger.error("Pool error occurred, attempting to reconnect", {
            error: error.message,
        });
        ...
        try {
            ...
            darkfateLogger.success("Pool reconnection successful");
        } catch (reconnectError) {
            darkfateLogger.error("Failed to reconnect pool", {
                error:
    ...
    async testConnection(): Promise<boolean> {
        ...
        try {
            ...
            darkfateLogger.error("Database connection test failed:", error);
            throw new Error(
    ...
    async cleanup(): Promise<void> {
        try {
            await this.pool.end();
            darkfateLogger.info("Database pool closed");
        } catch (error) {
            darkfateLogger.error("Error closing database pool:", error);
        }
    }
    ...
    async getAccountById(userId: UUID): Promise<Account | null> {
        return this.withDatabase(async () => {
            ...
            if (rows.length === 0) {
                darkfateLogger.debug("Account not found:", { userId });
                return null;
            }

            const account = rows[0];
            // darkfateLogger.debug("Account retrieved:", {
            //     userId,
    ...
    async createAccount(account: Account): Promise<boolean> {
        return this.withDatabase(async () => {
            try {
                ...
                darkfateLogger.debug("Account created successfully:", {
                    accountId,
                });
                return true;
            } catch (error) {
                darkfateLogger.error("Error creating account:", {
                    error:
    ...
    async getActorById(params: { roomId: UUID }): Promise<Actor[]> {
        return this.withDatabase(async () => {
            ...
            darkfateLogger.debug("Retrieved actors:", {
            ...
            return rows.map((row) => {
                try {
                    ...
                    darkfateLogger.warn("Failed to parse actor details:", {
                        actorId: row.id,
                        error:
                            error instanceof Error
                                ? error.message
            ...
            darkfateLogger.error("Failed to get actors:", {
                roomId: params.roomId,
                error: error.message,
            });
    ...
    async createMemory(memory: Memory, tableName: string): Promise<void> {
        return this.withDatabase(async () => {
            darkfateLogger.debug("PostgresAdapter createMemory:", {
                memoryId: memory.id,
    ...
    async getMemories(params: {
        ...
        return this.withDatabase(async () => {
            ...
            darkfateLogger.debug("Fetching memories:", {
                roomId: params.roomId,
                tableName: params.tableName,
                unique: params.unique,
                agentId: params.agentId,
    ...
    async updateGoal(goal: Goal): Promise<void> {
        return this.withDatabase(async () => {
            try {
                ...
                darkfateLogger.error("Failed to update goal:", {
                    goalId: goal.id,
                    error:
                        error instanceof Error ? error.message : String(error),
                    status: goal.status,
    ...
    async removeGoal(goalId: UUID): Promise<void> {
        ...
        return this.withDatabase(async () => {
            try {
                ...
                darkfateLogger.debug("Goal removal attempt:", {
                    goalId,
                    removed: result?.rowCount ?? 0 > 0,
                });
            } catch (error) {
                darkfateLogger.error("Failed to remove goal:", {
                    goalId,
                    error:
                        error instanceof Error ? error.message : String(error),
                });
        ...
        return this.withDatabase(async () => {
            ...
            try {
                ...
                if (checkResult.rowCount === 0) {
                    darkfateLogger.warn("No room found to remove:", { roomId });
                ...
                await client.query("ROLLBACK");
                darkfateLogger.error("Failed to remove room:", {
                    roomId,
                    error:
                        error instanceof Error ? error.message : String(error),
                });
    ...
    async createRelationship(params: {
        ...
        return this.withDatabase(async () => {
            try {
                ...
                darkfateLogger.debug("Relationship created successfully:", {
                    relationshipId,
                    userA: params.userA,
                    userB: params.userB,
                });
                ...
                if ((error as { code?: string }).code === "23505") {
                    // Unique violation
                    darkfateLogger.warn("Relationship already exists:", {
                        userA: params.userA,
                        userB: params.userB,
                        error:
                            error instanceof Error
                    ...
                    darkfateLogger.error("Failed to create relationship:", {
                        userA: params.userA,
                        userB: params.userB,
                        error:
                            error instanceof Error
    ...
    async getRelationship(params: {
        ...
        return this.withDatabase(async () => {
            try {
                ...
                if (rows.length > 0) {
                    darkfateLogger.debug("Relationship found:", {
                ...
                darkfateLogger.debug("No relationship found between users:", {
                    userA: params.userA,
                    userB: params.userB,
                });
                return null;
            } catch (error) {
                darkfateLogger.error("Error fetching relationship:", {
                    userA: params.userA,
                    userB: params.userB,
                    error:
                        error instanceof Error ? error.message : String(error),
    ...
    async getRelationships(params: { userId: UUID }): Promise<Relationship[]> {
        ...
        return this.withDatabase(async () => {
            try {
                ...
                darkfateLogger.debug("Retrieved relationships:", {
                ...
                darkfateLogger.error("Failed to fetch relationships:", {
                    userId: params.userId,
                    error:
                        error instanceof Error ? error.message : String(error),
                });
    ...
    async getCachedEmbeddings(opts: {
        ...
        return this.withDatabase(async () => {
            try {
                darkfateLogger.debug("Fetching cached embeddings:", {
                ...
                darkfateLogger.debug("Retrieved cached embeddings:", {
                    count: rows.length,
                    tableName: opts.query_table_name,
                    matchCount: opts.query_match_count,
                });
                ...
                darkfateLogger.error("Error in getCachedEmbeddings:", {
                    error:
                        error instanceof Error ? error.message : String(error),
                    tableName: opts.query_table_name,
                    fieldName: opts.query_field_name,
    ...
    async log(params: {
        ...
        return this.withDatabase(async () => {
            try {
                ...
                darkfateLogger.debug("Log entry created:", {
                    logId,
                    type: params.type,
                    roomId: params.roomId,
                    userId: params.userId,
                ...
                darkfateLogger.error("Failed to create log entry:", {
                    error:
                        error instanceof Error ? error.message : String(error),
                    type: params.type,
                    roomId: params.roomId,
    ...
    async searchMemoriesByEmbedding(
        ...
        return this.withDatabase(async () => {
            darkfateLogger.debug("Incoming vector:", {
            ...
            darkfateLogger.debug("Vector debug:", {
            ...
            darkfateLogger.debug("Query debug:", {
                sql: sql.slice(0, 200),
    ...
    async getActorDetails(params: { roomId: string }): Promise<Actor[]> {
        ...
        return this.withDatabase(async () => {
            try {
                ...
                darkfateLogger.debug("Retrieved actor details:", {
                    roomId: params.roomId,
                ...
                darkfateLogger.error("Failed to fetch actor details:", {
                    roomId: params.roomId,
                    error:
                        error instanceof Error ? error.message : String(error),
                });
    ...
    async getCache(params: {
        ...
        return this.withDatabase(async () => {
            try {
                ...
                darkfateLogger.error("Error fetching cache", {
                    error:
                        error instanceof Error ? error.message : String(error),
                    key: params.key,
                    agentId: params.agentId,
        ...
        return this.withDatabase(async () => {
            try {
                ...
                try {
                    ...
                    await client.query("ROLLBACK");
                    darkfateLogger.error("Error setting cache", {
                        error:
                            error instanceof Error
                                ? error.message
                                : String(error),
        ...
        return this.withDatabase(async () => {
            try {
                ...
                try {
                    ...
                    await client.query("ROLLBACK");
                    darkfateLogger.error("Error deleting cache", {
                        error:
                            error instanceof Error
                                ? error.message
                                : String(error),
