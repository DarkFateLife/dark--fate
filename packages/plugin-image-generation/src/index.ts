...
import { generateImage } from "@DarkFateLife/darkfate";
...
const imageGeneration: Action = {
    ...
    handler: async (
        ...
        darkfateLogger.log("Composing state for message:", message);
        state = (await runtime.composeState(message)) as State;
        const userId = runtime.agentId;
        darkfateLogger.log("User ID:", userId);

        const imagePrompt = message.content.text;
        darkfateLogger.log("Image prompt received:", imagePrompt);

        // TODO: Generate a prompt for the image

        const res: { image: string; caption: string }[] = [];

        darkfateLogger.log("Generating image with prompt:", imagePrompt);
        ...
        if (images.success && images.data && images.data.length > 0) {
            ...
            for (let i = 0; i < images.data.length; i++) {
                ...
                darkfateLogger.log(`Processing image ${i + 1}:`, filename);
                ...
                try {
                    ...
                    darkfateLogger.error("Caption generation failed, using default caption:", error);
                }*/
            ...
            darkfateLogger.error("Image generation failed or returned no data.");
        }
    },
    examples: [
