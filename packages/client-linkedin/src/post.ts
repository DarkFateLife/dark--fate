...
export class LinkedInPostClient {
  ...
  async start(postImmediately = false) {
    ...
    const generateNewPostLoop = async () => {
      ...
      darkfateLogger.log(`Next LinkedIn post scheduled in ${randomHours} hours`);
    };
  ...
  async generateNewPost() {
    darkfateLogger.log('Generating new LinkedIn post');
...
---`;
      ...
      darkfateLogger.debug('Generate post prompt:\n' + context);
      ...
      if (this.runtime.getSetting('LINKEDIN_DRY_RUN') === 'true') {
        darkfateLogger.info(`Dry run: would have posted: ${newPostContent}`);
        return;
      }

      try {
        darkfateLogger.log(`Posting new LinkedIn post:\n${newPostContent}`);
        ...
        darkfateLogger.log(`LinkedIn post created: ${result.url}`);
      } catch (error) {
        darkfateLogger.error('Error creating LinkedIn post:', error);
      }
    } catch (error) {
      darkfateLogger.error('Error generating new LinkedIn post:', error);
    }
  }
}