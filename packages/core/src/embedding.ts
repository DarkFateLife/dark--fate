...
import darkfateLogger from "./logger.ts";
...
async function getRemoteEmbedding(
    ...
    try {
        ...
        if (!response.ok) {
            darkfateLogger.error("API Response:", await response.text()); // Debug log
            throw new Error(
        ...
        darkfateLogger.error("Full error details:", e);
        throw e;
    }
}
...
export async function embed(runtime: IAgentRuntime, input: string) {
    darkfateLogger.debug("Embedding request:", {
    ...
    if (!input || typeof input !== "string" || input.trim().length === 0) {
        darkfateLogger.warn("Invalid embedding input:", {
            input,
            type: typeof input,
            length: input?.length,
        });
    ...
    async function getLocalEmbedding(input: string): Promise<number[]> {
        darkfateLogger.debug("DEBUG - Inside getLocalEmbedding function");
        ...
        try {
            const moduleImports = await Promise.all([
                ...
                (async () => {
                    try {
                        ...
                        darkfateLogger.error("Failed to load fastembed.");
            ...
            function getRootPath() {
                ...
                if (rootPath.includes("/darkfate/")) {
                    return rootPath.split("/darkfate/")[0] + "/darkfate/";
                }
            ...
            darkfateLogger.debug("Initializing BGE embedding model...");
            ...
            darkfateLogger.debug("Generating embedding for input:", {
                inputLength: input.length,
            ...
            darkfateLogger.debug("Raw embedding from BGE:", {
            ...
            darkfateLogger.debug("Processed embedding:", {
    ...
    async function retrieveCachedEmbedding(
        ...
        if (!input) {
            darkfateLogger.log("No input to retrieve cached embedding for");
            return null;
        }
