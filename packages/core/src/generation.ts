...
import { darkfateLogger } from "./index.ts";
...
export async function generateText({
    ...
    darkfateLogger.log("Generating text...");

    darkfateLogger.info("Generating text with options:", {
    ...
    darkfateLogger.info("Selected model:", model);
    ...
    try {
        ...
        switch (provider) {
            ...
            case ModelProviderName.TOGETHER: {
                darkfateLogger.debug("Initializing OpenAI model.");
                ...
                darkfateLogger.debug("Received response from OpenAI model.");
                break;
            }
            ...
            case ModelProviderName.GOOGLE: {
                ...
                darkfateLogger.debug("Received response from Google model.");
                break;
            }

            case ModelProviderName.ANTHROPIC: {
                darkfateLogger.debug("Initializing Anthropic model.");
                ...
                darkfateLogger.debug("Received response from Anthropic model.");
                break;
            }

            case ModelProviderName.CLAUDE_VERTEX: {
                darkfateLogger.debug("Initializing Claude Vertex model.");
            ...
            case ModelProviderName.GROK: {
                darkfateLogger.debug("Initializing Grok model.");
                ...
                darkfateLogger.debug("Received response from Grok model.");
                break;
            }
            ...
            case ModelProviderName.LLAMALOCAL: {
                ...
                darkfateLogger.debug("Received response from local Llama model.");
                break;
            }

            case ModelProviderName.REDPILL: {
                darkfateLogger.debug("Initializing RedPill model.");
                ...
                darkfateLogger.debug("Received response from redpill model.");
                break;
            }

            case ModelProviderName.OPENROUTER: {
                darkfateLogger.debug("Initializing OpenRouter model.");
                const serverUrl = models[provider].endpoint;
                ...
                darkfateLogger.debug("Received response from OpenRouter model.");
                break;
            }

            case ModelProviderName.OLLAMA:
                {
                    darkfateLogger.debug("Initializing Ollama model.");
                ...
                darkfateLogger.debug("Received response from Ollama model.");
                break;

            case ModelProviderName.HEURIST: {
                darkfateLogger.debug("Initializing Heurist model.");
                ...
                darkfateLogger.debug("Received response from Heurist model.");
                break;
            }
            case ModelProviderName.GAIANET: {
                darkfateLogger.debug("Initializing GAIANET model.");
                ...
                darkfateLogger.debug("Using GAIANET model with baseURL:", baseURL);
                ...
                response = openaiResponse;
                darkfateLogger.debug("Received response from GAIANET model.");
                break;
            }

            case ModelProviderName.GALADRIEL: {
                darkfateLogger.debug("Initializing Galadriel model.");
                ...
                darkfateLogger.debug("Received response from Galadriel model.");
                break;
            }

            default: {
                const errorMessage = `Unsupported provider: ${provider}`;
                darkfateLogger.error(errorMessage);
        ...
        darkfateLogger.error("Error in generateText:", error);
        throw error;
    }
}
...
export async function generateShouldRespond({
    ...
    while (true) {
        try {
            ...
            darkfateLogger.debug("Received response from generateText:", response);
            const parsedResponse = parseShouldRespondFromText(response.trim());
            if (parsedResponse) {
                darkfateLogger.debug("Parsed response:", parsedResponse);
                return parsedResponse;
            } else {
                darkfateLogger.debug("generateShouldRespond no response");
            }
        } catch (error) {
            darkfateLogger.error("Error in generateShouldRespond:", error);
            if (
        ...
        darkfateLogger.log(`Retrying in ${retryDelay}ms...`);
...
export async function generateTrueOrFalse({
    ...
    while (true) {
        try {
            ...
            darkfateLogger.error("Error in generateTrueOrFalse:", error);
        }
...
export async function generateTextArray({
    ...
    if (!context) {
        darkfateLogger.error("generateTextArray context is empty");
        return [];
    }
    let retryDelay = 1000;
    ...
    while (true) {
        try {
            ...
            darkfateLogger.error("Error in generateTextArray:", error);
        }
...
export async function generateObjectDEPRECATED({
    ...
    if (!context) {
        darkfateLogger.error("generateObjectDEPRECATED context is empty");
        return null;
    }
    let retryDelay = 1000;
    ...
    while (true) {
        try {
            ...
            darkfateLogger.error("Error in generateObject:", error);
        }
...
export async function generateObjectArray({
    ...
    if (!context) {
        darkfateLogger.error("generateObjectArray context is empty");
        return [];
    }
    let retryDelay = 1000;
...
export async function generateMessageResponse({
    ...
    while (true) {
        try {
            darkfateLogger.log("Generating message response..");
            ...
            if (!parsedContent) {
                darkfateLogger.debug("parsedContent is null, retrying");
                continue;
            }

            return parsedContent;
        } catch (error) {
            darkfateLogger.error("ERROR:", error);
            // wait for 2 seconds
            retryLength *= 2;
            await new Promise((resolve) => setTimeout(resolve, retryLength));
            darkfateLogger.debug("Retrying...");
        }
    }
}
...
export const generateImage = async (
    ...
    darkfateLogger.info("Generating image with options:", {
        ...
        if (runtime.imageModelProvider === ModelProviderName.HEURIST) {
            ...
            const base64s = await Promise.all(
                togetherResponse.data.map(async (image) => {
                    if (!image.url) {
                        darkfateLogger.error("Missing URL in image data:", image);
            ...
            darkfateLogger.debug(`Generated ${base64s.length} images`);
            ...
            const result = await fal.subscribe(model, {
                ...
                onQueueUpdate: (update) => {
                    if (update.status === "IN_PROGRESS") {
                        darkfateLogger.info(update.logs.map((log) => log.message));
                    }
                },
            });
...
export const generateWebSearch = async (
    ...
    try {
        ...
        if (!response.ok) {
            throw new darkfateLogger.error(
