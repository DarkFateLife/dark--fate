...
async function get(
    ...
    if (!message?.content?.text) {
        darkfateLogger.warn("Invalid message for knowledge query:", {
            message,
    ...
    darkfateLogger.debug("Knowledge query:", {
    ...
    if (!processed || processed.trim().length === 0) {
        darkfateLogger.warn("Empty processed text for knowledge query");
        return [];
    }
...
export function preprocess(content: string): string {
    darkfateLogger.debug("Preprocessing text:", {
        input: content,
        length: content?.length,
    });

    if (!content || typeof content !== "string") {
        darkfateLogger.warn("Invalid input for preprocessing");
        return "";
    }
