...
export class AgentRuntime implements IAgentRuntime {
    ...
    getService<T extends Service>(service: ServiceType): T | null {
        ...
        if (!serviceInstance) {
            darkfateLogger.error(`Service ${service} not found`);
            return null;
        }
    ...
    async registerService(service: Service): Promise<void> {
        ...
        darkfateLogger.log("Registering service:", serviceType);
        ...
        darkfateLogger.success(`Service ${serviceType} registered successfully`);
    }
    ...
    constructor(opts: {
        ...
        darkfateLogger.info("Initializing AgentRuntime with options:", {
        ...
        darkfateLogger.success("Agent ID", this.agentId);
        ...
        darkfateLogger.info("Setting model provider...");
        darkfateLogger.info("Model Provider Selection:", {
        ...
        darkfateLogger.info("Selected model provider:", this.modelProvider);
        darkfateLogger.info(
        ...
        if (!Object.values(ModelProviderName).includes(this.modelProvider)) {
            darkfateLogger.error("Invalid model provider:", this.modelProvider);
        ...
        if (!this.serverUrl) {
            darkfateLogger.warn("No serverUrl provided, defaulting to localhost");
        }
    ...
    registerAction(action: Action) {
        darkfateLogger.success(`Registering action: ${action.name}`);
    ...
    async processActions(
        ...
        for (const response of responses) {
            if (!response.content?.action) {
                darkfateLogger.warn("No action found in the response content.");
                continue;
            }
            ...
            darkfateLogger.success(`Normalized action: ${normalizedAction}`);
            ...
            if (!action) {
                darkfateLogger.info("Attempting to find action in similes.");
            ...
            if (!action.handler) {
                darkfateLogger.error(`Action ${action.name} has no handler.`);
                continue;
            }
    ...
    async evaluate(
        ...
        const evaluatorPromises = this.evaluators.map(
            async (evaluator: Evaluator) => {
                darkfateLogger.log("Evaluating", evaluator.name);
    ...
    async ensureUserExists(
        ...
        if (!account) {
            ...
            darkfateLogger.success(`User ${userName} created successfully.`);
        }
    }
    ...
    async ensureRoomExists(roomId: UUID) {
        ...
        if (!room) {
            ...
            darkfateLogger.log(`Room ${roomId} created successfully.`);
        }
    }
