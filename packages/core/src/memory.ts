...
export class MemoryManager implements IMemoryManager {
    ...
    async addEmbeddingToMemory(memory: Memory): Promise<Memory> {
        ...
        try {
            ...
            darkfateLogger.error("Failed to generate embedding:", error);
    ...
    async createMemory(memory: Memory, unique = false): Promise<void> {
        ...
        if (existingMessage) {
            darkfateLogger.debug("Memory already exists, skipping");
            return;
        }

        darkfateLogger.log("Creating Memory", memory.id, memory.content.text);
