...
const generateVideo = async (prompt: string, runtime: IAgentRuntime) => {
    ...
    try {
        darkfateLogger.log("Starting video generation with prompt:", prompt);
        ...
        if (!response.ok) {
            ...
            darkfateLogger.error("Luma API error:", {
        ...
        while (status !== "completed" && status !== "failed") {
            ...
            if (!statusResponse.ok) {
                ...
                darkfateLogger.error("Status check error:", {
            ...
            darkfateLogger.log("Status check response:", statusData);
        ...
        darkfateLogger.error("Video generation error:", error);
        return {
            success: false,
...
const videoGeneration: Action = {
    ...
    validate: async (runtime: IAgentRuntime, _message: Memory) => {
        darkfateLogger.log("Validating video generation action");
        const lumaApiKey = runtime.getSetting("LUMA_API_KEY");
        darkfateLogger.log("LUMA_API_KEY present:", !!lumaApiKey);
        return !!lumaApiKey;
    },
    handler: async (
        ...
        darkfateLogger.log("Video generation request:", message);
        ...
        darkfateLogger.log("Video prompt:", videoPrompt);
        ...
        try {
            ...
            darkfateLogger.error(`Failed to generate video. Error: ${error}`);
            callback({
