...
export class FarcasterInteractionManager {
    ...
    private async handleCast({
        ...
        if (cast.profile.fid === agent.fid) {
            darkfateLogger.info("skipping cast from bot itself", cast.hash);
            return;
        }

        if (!memory.content.text) {
            darkfateLogger.info("skipping cast with no text", cast.hash);
        ...
        try {
            darkfateLogger.info(`Replying to cast ${cast.hash}.`);
            ...
            darkfateLogger.error(`Error sending response cast: ${error}`);
        }
    }
}
