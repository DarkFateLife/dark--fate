...
export class FarcasterPostManager {
    ...
    private async generateNewCast() {
        darkfateLogger.info("Generating new cast");
        try {
            ...
            try {
                ...
                darkfateLogger.info(`[Farcaster Neynar Client] Published cast ${cast.hash}`);
                ...
                darkfateLogger.error("Error sending cast:", error)
            }
        } catch (error) {
            darkfateLogger.error("Error generating new cast:", error)
        }
    }
}
