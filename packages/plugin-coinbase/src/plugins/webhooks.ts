...
export const webhookProvider: Provider = {
    get: async (runtime: IAgentRuntime, _message: Memory) => {
        try {
            ...
            darkfateLogger.log("Listing all webhooks:", resp.data);
            ...
            darkfateLogger.error("Error in webhookProvider:", error);
            return [];
        }
    },
};
...
export const createWebhookAction: Action = {
    ...
    validate: async (runtime: IAgentRuntime, _message: Memory) => {
        darkfateLogger.log("Validating runtime for CREATE_WEBHOOK...");
        return (
            !!(
    ...
    handler: async (
        ...
        darkfateLogger.log("Starting CREATE_WEBHOOK handler...");
        ...
        try {
            ...
            darkfateLogger.log("Creating webhook with details:", {networkId, notificationUri, eventType, eventTypeFilter, eventFilters});
            const webhook = await Webhook.create({networkId, notificationUri, eventType, eventFilters});
            darkfateLogger.log("Webhook created successfully:", webhook.toString());
            callback(
                {
            ...
            darkfateLogger.log("Webhook appended to CSV successfully");
        } catch (error) {
            darkfateLogger.error("Error during webhook creation:", error);
            callback(
                {
