    ...
    get: async (runtime: IAgentRuntime, _message: Memory) => {
        try {
            ...
            try {
                ...
                darkfateLogger.error("Error fetching accounts:", error);
                return [];
            }
            ...
            try {
                ...
                darkfateLogger.error("Error fetching products:", error);
                return [];
            }
            ...
            try {
                ...
                darkfateLogger.error("Error reading CSV file:", error);
                return [];
            }
            ...
            try {
                ...
                darkfateLogger.error("Error parsing CSV data:", error);
                return [];
            }
            ...
            darkfateLogger.error("Error in tradeProvider:", error);
            return [];
        }
    },
};
...
export async function appendTradeToCsv(tradeResult: any) {
    try {
        ...
        darkfateLogger.info("Trade result:", tradeResult);
        ...
        darkfateLogger.info("Formatted trade for CSV:", formattedTrade);
        await csvWriter.writeRecords([formattedTrade]);
        darkfateLogger.info("Trade written to CSV successfully");
    } catch (error) {
        darkfateLogger.error("Error writing trade to CSV:", error);
        // Log the actual error for debugging
        if (error instanceof Error) {
            darkfateLogger.error("Error details:", error.message);
        }
    }
}
...
async function hasEnoughBalance(client: RESTClient, currency: string, amount: number, side: string): Promise<boolean> {
    try {
        ...
        darkfateLogger.info("Accounts:", accounts);
        const checkCurrency = side === "BUY" ? "USD" : currency;
        darkfateLogger.info(`Checking balance for ${side} order of ${amount} ${checkCurrency}`);
        ...
        if (!account) {
            darkfateLogger.error(`No ${checkCurrency} account found`);
            return false;
        }
        ...
        darkfateLogger.info(`Required amount (including buffer): ${requiredAmount} ${checkCurrency}`);

        const hasBalance = available >= requiredAmount;
        darkfateLogger.info(`Has sufficient balance: ${hasBalance}`);

        return hasBalance;
    } catch (error) {
        darkfateLogger.error("Balance check failed with error:", {
...
export const executeAdvancedTradeAction: Action = {
    ...
    handler: async (
        ...
        try {
            ...
            darkfateLogger.info("Advanced trade client initialized");
        } catch (error) {
            darkfateLogger.error("Client initialization failed:", error);
            callback({
        ...
        try {
            ...
            darkfateLogger.info("Trade details generated:", tradeDetails.object);
        } catch (error) {
            darkfateLogger.error("Trade details generation failed:", error);
            callback({
        ...
        if (!isAdvancedTradeContent(tradeDetails.object)) {
            darkfateLogger.error("Invalid trade content:", tradeDetails.object);
            callback({
        ...
        try {
            ...
            darkfateLogger.info("Order configuration created:", orderConfiguration);
        } catch (error) {
            darkfateLogger.error("Order configuration failed:", error);
            callback({
        ...
        try {
            ...
            darkfateLogger.info("Trade executed successfully:", order);
        } catch (error) {
            darkfateLogger.error("Trade execution failed:", error?.message);
            callback({
            ...
            try {
                ...
                darkfateLogger.info("Trade logged to CSV");
            } catch (csvError) {
                darkfateLogger.warn("Failed to log trade to CSV:", csvError);
