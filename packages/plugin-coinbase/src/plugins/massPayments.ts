...
export const massPayoutProvider: Provider = {
    get: async (runtime: IAgentRuntime, _message: Memory) => {
        try {
            ...
            darkfateLogger.log("Reading CSV file from:", csvFilePath);
            ...
            darkfateLogger.log("Parsed CSV records:", records);
            darkfateLogger.log("Current Balances:", balances);
            darkfateLogger.log("Last Transactions:", transactions);
            ...
            darkfateLogger.error("Error in massPayoutProvider:", error);
...
async function executeMassPayout(
    ...
    try {
        ...
        darkfateLogger.error("Error initializing sending wallet:", error);
        throw error;
    }
    for (const address of receivingAddresses) {
        darkfateLogger.log("Processing payout for address:", address);
            if (address) {
                try {
                    ...
                    darkfateLogger.log("Wallet balance for asset:", {
                        assetId,
                        walletBalance,
                    });
                    ...
                    if (walletBalance.lessThan(transferAmount)) {
                        ...
                        darkfateLogger.error(insufficientFunds);
                ...
                darkfateLogger.log("Skipping invalid or empty address.");
                transactions.push({
        ...
        try {
            ...
            darkfateLogger.error("Error during charity transfer:", error);
            transactions.push({
                address: charityAddress,
        ...
        darkfateLogger.log("Finished processing mass payouts.");
    return transactions;
}
...
export const sendMassPayoutAction: Action = {
    ...
    validate: async (runtime: IAgentRuntime, _message: Memory) => {
        darkfateLogger.log("Validating runtime and message...");
        return (
            !!(
    ...
    handler: async (
        ...
        darkfateLogger.log("Starting SEND_MASS_PAYOUT handler...");
        try {
            Coinbase.configure({
                apiKeyName:
                    runtime.getSetting("COINBASE_API_KEY") ??
            ...
            if (
                ...
                darkfateLogger.error("Missing or invalid input parameters:", {
                    network,
                    receivingAddresses,
                    transferAmount,
                    assetId,
...
- Asset ID is valid.`,
            ...
            darkfateLogger.log("◎ Starting mass payout...");
...
Check the CSV file for full details.`,
            ...
            darkfateLogger.error("Error during mass payouts:", error);
            callback(
