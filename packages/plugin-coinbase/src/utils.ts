...
import { darkfateLogger, IAgentRuntime } from "@DarkFateLife/darkfate";
import fs from "fs";
    ...
    if (!storedSeed || !storedWalletId) {
        ...
        try {
            ...
            if (walletIDSave && seedSave) {
                darkfateLogger.log("Successfully updated character secrets.");
            } else {
            ...
            darkfateLogger.log("Wallet created and stored new wallet:", walletAddress);
        } catch (error) {
            darkfateLogger.error("Error updating character secrets:", error);
            throw error;
        }

        // Logging wallet creation
        darkfateLogger.log("Created and stored new wallet:", walletAddress);
    } else {
        ...
        darkfateLogger.log("Imported existing wallet for network:", networkId);
...
export async function executeTradeAndCharityTransfer(runtime: IAgentRuntime, network: string, amount: number, sourceAsset: string, targetAsset: string) {
    ...
    darkfateLogger.log("Wallet initialized:", {
        network,
    ...
    if (charityAddress && charityAmount > 0) {
        ...
        darkfateLogger.log("Charity Transfer successful:", {
    ...
    darkfateLogger.log("Trade initiated:", trade.toString());
    await trade.wait();
    darkfateLogger.log("Trade completed successfully:", trade.toString());
...
export async function appendTradeToCsv(trade: Trade) {
    try {
        ...
        darkfateLogger.log("Writing trade to CSV:", formattedTrade);
        await csvWriter.writeRecords([formattedTrade]);
        darkfateLogger.log("Trade written to CSV successfully.");
    } catch (error) {
        darkfateLogger.error("Error writing trade to CSV:", error);
    }
}
...
export async function appendTransactionsToCsv(transactions: Transaction[]) {
    try {
        ...
        darkfateLogger.log("Writing transactions to CSV:", formattedTransactions);
        await csvWriter.writeRecords(formattedTransactions);
        darkfateLogger.log("All transactions written to CSV successfully.");
    } catch (error) {
        darkfateLogger.error("Error writing transactions to CSV:", error);
    }
}
...
export async function appendWebhooksToCsv(webhooks: Webhook[]) {
    try {
        ...
        if (!fs.existsSync(webhookCsvFilePath)) {
            darkfateLogger.warn("CSV file not found. Creating a new one.");
            const csvWriter = createArrayCsvWriter({
            ...
            darkfateLogger.log("New CSV file created with headers.");
                    }
        const csvWriter = createArrayCsvWriter({
            path: webhookCsvFilePath,
            header: [
        ...
        darkfateLogger.log("Writing webhooks to CSV:", formattedWebhooks);
        await csvWriter.writeRecords(formattedWebhooks);
        darkfateLogger.log("All webhooks written to CSV successfully.");
    } catch (error) {
        darkfateLogger.error("Error writing webhooks to CSV:", error);
    }
}
...
export async function updateCharacterSecrets(
    ...
    try {
        ...
        if (!fs.existsSync(characterFilePath)) {
            darkfateLogger.error("Character file not found:", characterFilePath);
            return false;
        }
...
export async function executeTransferAndCharityTransfer(wallet: Wallet, amount: number, sourceAsset: string, targetAddress: string, network: string) {
    ...
    if (charityAddress && charityAmount > 0) {
        ...
        darkfateLogger.log("Charity Transfer successful:", charityTransfer.toString());
    }
    ...
    darkfateLogger.log("Initiating transfer:", transferDetails);
    const transfer = await wallet.createTransfer(transferDetails);
    darkfateLogger.log("Transfer initiated:", transfer.toString());
    await transfer.wait();
...
- Charity Transaction URL: ${charityTransfer.getTransactionLink() || ""}`;
    ...
    darkfateLogger.log(responseText);
...
export async function executeTransfer(wallet: Wallet, amount: number, sourceAsset: string, targetAddress: string) {
    ...
    try {
        ...
        darkfateLogger.error("Error executing transfer:", error);
    }
    return transfer;
}
