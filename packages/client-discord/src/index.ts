import { getEmbeddingZeroVector } from "@DarkFateLife/darkfate";
import { Character, Client as ElizaClient, IAgentRuntime } from "@DarkFateLife/darkfate";
...
export class DiscordClient extends EventEmitter {
    ...
    private async onClientReady(readyClient: { user: { tag: any; id: any } }) {
        darkfateLogger.success(`Logged in as ${readyClient.user?.tag}`);
        ...
        try {
            ...
            darkfateLogger.success("Slash commands registered");
        } catch (error) {
        ...
        darkfateLogger.success("Use this URL to add the bot to your server:");
    ...
    async handleReactionAdd(reaction: MessageReaction, user: User) {
        try {
            darkfateLogger.log("Reaction added");

            // Early returns
            if (!reaction || !user) {
                darkfateLogger.warn("Invalid reaction or user");
                return;
            }
            ...
            if (!userIdUUID || !roomId) {
                darkfateLogger.error("Invalid user ID or room ID", {
                    userIdUUID,
                    roomId,
                });
                return;
            ...
            try {
                ...
                darkfateLogger.debug("Reaction memory created", {
                ...
                if (error.code === "23505") {
                    // Duplicate key error
                    darkfateLogger.warn("Duplicate reaction memory, skipping", {
                        reactionId: reactionUUID,
                    });
                    return;
                }
            ...
            darkfateLogger.error("Error handling reaction:", error);
        }
    }

    async handleReactionRemove(reaction: MessageReaction, user: User) {
        darkfateLogger.log("Reaction removed");
        // if (user.bot) return;
