...
export class AudioMonitor {
    ...
    constructor(
        ...
        this.readable.on("end", () => {
            darkfateLogger.log("AudioMonitor ended");
            this.ended = true;
        ...
        this.readable.on("speakingStopped", () => {
            ...
            darkfateLogger.log("Speaking stopped");
            if (this.lastFlagged < 0) return;
            callback(this.getBufferFromStart());
        });
        this.readable.on("speakingStarted", () => {
            if (this.ended) return;
            darkfateLogger.log("Speaking started");
            this.reset();
        });
    }
...
export class VoiceManager extends EventEmitter {
        ...
        try {
            ...
            connection.on("stateChange", async (oldState, newState) => {
                ...
                if (newState.status === VoiceConnectionStatus.Disconnected) {
                    darkfateLogger.log("Handling disconnection...");
                    ...
                    try {
                        ...
                        darkfateLogger.log("Reconnecting to channel...");
                    } catch (e) {
            ...
            connection.on("error", (error) => {
                darkfateLogger.log("Voice connection error:", error);
            ...
            if (me?.voice && me.permissions.has("DeafenMembers")) {
                try {
                    ...
                    darkfateLogger.log("Failed to modify voice state:", error);
            ...
            darkfateLogger.log("Failed to establish voice connection:", error);
            connection.destroy();
            this.connections.delete(channel.id);
            throw error;
        }
    ...
    private async _shouldIgnore(message: Memory): Promise<boolean> {
        ...
        darkfateLogger.debug("message.content: ", message.content);
