...
function isTransferContent(
    ...
    darkfateLogger.log("Content for transfer", content);
    return (
        (!content.token ||
...
export class TransferAction {
    ...
    async processMessages(
        ...
        if (!isTransferContent(runtime, content)) {
            darkfateLogger.error("Invalid content for SEND_COIN action.");
            throw new Exception(50100, "Invalid transfer content");
        }

        // Check if the content is matched
        if (!content.matched) {
            darkfateLogger.error("Content does not match the transfer template.");
            throw new Exception(
                50100,
    ...
    async transfer(
        ...
        darkfateLogger.log("Starting Flow Plugin's SEND_COIN handler...");
        ...
        if (totalBalance < amount) {
            darkfateLogger.error("Insufficient balance to transfer.");
            if (callback) {
                callback({
        ...
        try {
            ...
            darkfateLogger.log(`${logPrefix} Sent transaction: ${resp.txid}`);
            ...
            darkfateLogger.error("Error in sending transaction:", e.message);
            if (callback) {
                callback({
        ...
        darkfateLogger.log("Completed Flow Plugin's SEND_COIN handler.");
...
export const transferAction = {
    ...
    validate: async (runtime: IAgentRuntime, _message: Memory) => {
        ...
        try {
            ...
            darkfateLogger.error("Failed to sync account info");
            return false;
        }
        return true;
    },
    handler: async (
        ...
        try {
            ...
            darkfateLogger.error("Error in processing messages:", err.message);
            if (callback) {
                callback({
                    text:
