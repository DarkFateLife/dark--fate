...
export class FlowWalletProvider implements IFlowSigner, IFlowScriptExecutor {
    ...
    async syncAccountInfo() {
        ...
        darkfateLogger.debug("Flow account info synced", {
...
function getSignerAddress(runtime: IAgentRuntime): string {
    ...
    if (!signerAddr) {
        darkfateLogger.error("No signer address");
    ...
    get: async (
        ...
        try {
            ...
            if (!info || info?.address !== walletProvider.address) {
                darkfateLogger.error("Invalid account info");
                return null;
            }
            ...
            darkfateLogger.error("Error in Flow wallet provider:", error.message);
            return null;
        }
    },
};
