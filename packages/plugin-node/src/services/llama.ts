...
import { Service } from "@DarkFateLife/darkfate";
import fs from "fs";
...
export class LlamaService extends Service {
    ...
    async initialize(runtime: IAgentRuntime): Promise<void> {
        darkfateLogger.info("Initializing LlamaService...");
    ...
    private async ensureInitialized() {
        if (!this.modelInitialized) {
            ...
            darkfateLogger.info("Model already initialized");
        }
    }

    async initializeModel() {
        try {
            darkfateLogger.info("Checking model file...");
            ...
            darkfateLogger.info("Initializing Llama instance...");
            this.llama = await getLlama({
                gpu: hasCUDA ? "cuda" : undefined,
            });

            darkfateLogger.info("Creating JSON schema grammar...");
            ...
            darkfateLogger.info("Loading model...");
            this.model = await this.llama.loadModel({
                modelPath: this.modelPath,
            });

            darkfateLogger.info("Creating context and sequence...");
            this.ctx = await this.model.createContext({ contextSize: 8192 });
            this.sequence = this.ctx.getSequence();

            this.modelInitialized = true;
            darkfateLogger.success("Model initialization complete");
            this.processQueue();
        } catch (error) {
    ...
    async checkModel() {
        if (!fs.existsSync(this.modelPath)) {
            darkfateLogger.info("Model file not found, starting download...");
                    ...
                    https
                        .get(url, (response) => {
                            ...
                            response.on("data", (chunk) => {
                                ...
                                darkfateLogger.progress(progressString);
                            });

                            file.on("finish", () => {
                                file.close();
                                darkfateLogger.progress(""); // Clear the progress line
                                darkfateLogger.success("Model download complete");
                                resolve();
                            });
            ...
            darkfateLogger.warn("Model already exists.");
        }
    }
    ...
    async completion(prompt: string, runtime: IAgentRuntime): Promise<string> {
        try {
            ...
            darkfateLogger.error("Error in completion:", error);
            throw error;
        }
    }
    ...
    async embedding(text: string, runtime: IAgentRuntime): Promise<number[]> {
        try {
            ...
            darkfateLogger.error("Error in embedding:", error);
            throw error;
        }
    }
    ...
    private async getCompletionResponse(
        ...
        for await (const token of this.sequence.evaluate(tokens, {
            ...
            if ([...stop].some((s) => current.includes(s))) {
                darkfateLogger.info("Stop sequence found");
                break;
            }
            ...
            if (useGrammar) {
                if (current.replaceAll("\n", "").includes("}```")) {
                    darkfateLogger.info("JSON block found");
                    break;
                }
            }
            if (responseTokens.length > max_tokens) {
                darkfateLogger.info("Max tokens reached");
                break;
            }
        }
        ...
        if (useGrammar) {
            ...
            try {
                ...
                darkfateLogger.error("Error parsing JSON:", error);
            }
        } else {
            await this.sequence.clearHistory();
            return response;
