import { darkfateLogger, models } from "@DarkFateLife/darkfate";
    ...
    implements IImageDescriptionService
{
    ...
    private async initializeLocalModel(): Promise<void> {
        ...
        darkfateLogger.info("Downloading Florence model...");
        ...
        darkfateLogger.success("Florence model downloaded successfully");

        darkfateLogger.info("Downloading processor...");
        this.processor = (await AutoProcessor.from_pretrained(
            this.modelId
        )) as Florence2Processor;

        darkfateLogger.info("Downloading tokenizer...");
        this.tokenizer = await AutoTokenizer.from_pretrained(this.modelId);
        darkfateLogger.success("Image service initialization complete");
    }
    ...
    private async recognizeWithOpenAI(
        ...
        try {
            ...
            darkfateLogger.error("Error in recognizeWithOpenAI:", error);
            throw error;
        }
    }
    ...
    private async processImage(
        ...
        darkfateLogger.log("Processing image:", imageUrl);
        const isGif = imageUrl.toLowerCase().endsWith(".gif");
        let imageToProcess = imageUrl;

        try {
            if (isGif) {
                darkfateLogger.log("Extracting first frame from GIF");
                const { filePath } =
                    await this.extractFirstFrameFromGif(imageUrl);
            ...
            darkfateLogger.log("Generating image description");
            ...
            darkfateLogger.error("Error processing image:", error);
            throw error;
        } finally {
