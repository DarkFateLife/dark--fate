...
import { Service, ServiceType } from "@DarkFateLife/darkfate";
    ...
    implements ITranscriptionService
{
    ...
    private async convertAudio(inputBuffer: ArrayBuffer): Promise<Buffer> {
        ...
        try {
            ...
            darkfateLogger.log("Input audio info:", stream);
            ...
            darkfateLogger.log("FFmpeg command:", ffmpegCommand);
            ...
            darkfateLogger.error("Error converting audio:", error);
            throw error;
        }
    }
    ...
    private async saveDebugAudio(audioBuffer: ArrayBuffer, prefix: string) {
        ...
        darkfateLogger.log(`Debug audio saved: ${filePath}`);
    }
    ...
    private async transcribeWithOpenAI(
        ...
        darkfateLogger.log("Transcribing audio with OpenAI...");
        ...
        try {
            ...
            darkfateLogger.log(`OpenAI speech to text result: "${trimmedResult}"`);
            ...
            if (error.response) {
                darkfateLogger.error("Response data:", error.response.data);
                darkfateLogger.error("Response status:", error.response.status);
                darkfateLogger.error("Response headers:", error.response.headers);
            } else if (error.request) {
                darkfateLogger.error("No response received:", error.request);
            } else {
                darkfateLogger.error("Error setting up request:", error.message);
            }
            return null;
        }
    }
    ...
    public async transcribeLocally(
        ...
        try {
            darkfateLogger.log("Transcribing audio locally...");
            ...
            darkfateLogger.debug(`Temporary WAV file created: ${tempWavFile}`);
            ...
            if (!output || output.length < 5) {
                darkfateLogger.log("Output is null or too short, returning null");
                return null;
            }
            return output;
        } catch (error) {
