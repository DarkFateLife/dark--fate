...
export class WalletProvider {
    ...
    async getWalletPortfolio(): Promise<TokenBalances> {
        ...
        if (cachedValues) {
            darkfateLogger.debug("Using cached data for getWalletPortfolio()");
            return cachedValues;
        }
    ...
    async getTokenUsdValues(): Promise<CoingeckoPrices> {
        ...
        if (cachedValues) {
            darkfateLogger.debug("Using cached data for getTokenUsdValues()");
            return cachedValues;
        }
...
const walletProvider: Provider = {
    get: async (
        ...
        try {
            ...
            darkfateLogger.error("Error in walletProvider.get():", error);
