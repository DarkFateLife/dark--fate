...
docker build -t username/darkfate-agent:latest .

# For architecture other than AMD64, run
docker buildx build --platform=linux/amd64 -t username/darkfate-agent:latest .
```

For Linux/AMD64 machines, you can now test the agent locally by updating the `TEE_MODE` environment variable to `DOCKER` and setting the environment variables in the [docker-compose.yaml](https://github.com/DarkFateLife/darkfate/blob/main/docker-compose.yaml) file. Once you have done that, you can start the agent by running:
...
```bash
docker push username/darkfate-agent:latest
```
...
services:
    tee:
        ...
        image: username/darkfate-agent:latest
        stdin_open: true
        tty: true
        volumes:
