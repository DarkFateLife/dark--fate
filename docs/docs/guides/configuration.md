...
const result = await generateImage(
  {
    prompt:
      'A cute anime girl with big breasts and straight long black hair wearing orange T-shirt. The T-shirt has "DarkFateLife" texts in the front. The girl is looking at the viewer',
    width: 1024,
    height: 1024,
...
3. Configure in `darkfateConfig.yaml`:
...
Enable and configure plugins in `darkfateConfig.yaml`:
