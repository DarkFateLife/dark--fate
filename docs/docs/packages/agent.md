...
The Agent Package (`@darkfate/agent`) provides the high-level orchestration layer for Eliza, managing agent lifecycles, character loading, client initialization, and runtime coordination.
...
The Agent Package (`@DarkFateLife/agent`) serves as the orchestration layer for Eliza, handling:
...
```bash
pnpm add @DarkFateLife/agent
```

## Quick Start

```typescript
import { startAgents, loadCharacters } from "@DarkFateLife/agent";
