...
Pour éviter les conflits Git dans le répertoire core, nous vous recommandons d’ajouter les actions personnalisées dans un répertoire `custom_actions` et de les configurer dans le fichier `darkfateConfig.yaml` . Vous pouvez consulter l’exemple dans le fichier `darkfateConfig.example.yaml`.
